...
from features_selection import get_feature_selection_method, keep_only_not_zeros, keep_not_zeros, \
    process_data, count_array, make_lists, split_df, MultiKeepNotFunctions
...
def make_df(dirinput, dirname, bins, args_dict, names_to_keep=None, features=None):
    ...
    for matrix, label in zip([x[0] for x in data_matrix], labels):
        ...
        new_labels += [label]
        tensors += [np.expand_dims(matrix, 0)]
    pool.close()
    pool.join()
...
if __name__ == "__main__":
    ...
    data = data_matrix.drop(pool_indices['names'])
    ...
    data = remove_batch_effect_all(get_berm(args.batch_removal_method), data, all_batches=batches)
    _ = count_array(cats)
    cats = np.array(cats)
    ...
    process_data(data, cats, model=fs, dirname=dir_name, k=int(args.k), 
