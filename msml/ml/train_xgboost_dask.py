...
class Train:
    ...
    def train(self, h_params):
        ...
        while h < self.args.n_repeats:                                
            ...
            dtrain = dxgb.DaskQuantileDMatrix(train_data, label=lists['classes']['train'][-1])
            dvalid = dxgb.DaskQuantileDMatrix(valid_data, label=lists['classes']['valid'][-1])
            dtest = dxgb.DaskQuantileDMatrix(test_data, label=lists['classes']['test'][-1])
            eval_set = [(dtrain, 'train'), (dvalid, 'eval')]
            early_stop = dxgb.callback.EarlyStopping(int(param_grid['early_stopping_rounds']))
            ...
            if all_data['inputs']['urinespositives'].shape[0] > 0:
                durines = dxgb.DaskQuantileDMatrix(all_data['inputs']['urinespositives'])
    ...
    def train_no_split(self, h_params):
        ...
        if self.args.model_name != 'xgboost':
            ...
            train_data = dxgb.DaskQuantileDMatrix(train_data, label=lists['classes']['train'])
        try:
