...
class TrainAE:
    ...
    def __init__(self, args, path, fix_thres=-1, load_tb=False, log_metrics=False, keep_models=True, log_inputs=True,
                 log_plots=False, groupkfold=True, pools=True):
        """
    ...
    def log_rep(self, best_lists, best_vals, best_values, traces, metrics, run, loggers, ae, shap_ae, h,
        ...
        if self.log_metrics:
            if self.args.log_tb and self.log_metrics:
                try:
            ...
            if self.args.log_neptune and self.log_metrics:
                try:
                    ...
                    print("\n\n\nProblem with logging stuff!\n\n\n")
            if self.args.log_mlflow and self.log_metrics:
                try:
    ...
    def warmup_loop(self, optimizer_ae, scheduler, ae, celoss, loader, triplet_loss, mseloss, warmup, epoch,
        ...
        for i, all_batch in iterator:
            ...
            if self.args.max_norm > 0:
                ...
                nn.utils.clip_grad_norm_(ae.parameters(), max_norm=self.args.max_norm)
            optimizer_ae.step()
            if self.args.scheduler is not None and self.args.scheduler != 'ReduceLROnPlateau':
                scheduler.step()

        if self.args.scheduler == 'ReduceLROnPlateau':
            scheduler.step(np.mean(traces['rec_loss']) < self.best_loss)
    ...
    def ae_pures_loop(self, optimizer_ae, scheduler_c, ae, celoss, loader, triplet_loss, mseloss, warmup, epoch,
                    optimizer_b, values, loggers, loaders, run, mapping=True):
        lists, traces = get_empty_traces()
        ae.train()
        ae.mapper.train()
...
if __name__ == "__main__":
    ...
    train = TrainAE(args, args.path, fix_thres=-1, load_tb=False, log_metrics=True, keep_models=False,
                    log_inputs=False, log_plots=True, groupkfold=args.groupkfold, pools=True)
