...
from sklearn.preprocessing import MinMaxScaler, RobustScaler, Normalizer, StandardScaler, minmax_scale
...
def get_images_loaders_no_pool(data, random_recs, samples_weights, args, 
                               ae=None, classifier=None, device='cuda'):
    """
    ...
    train_set = MSDataset4(data['inputs']['train'],
                           data['meta']['train'], 
                           data['names']['train'],
                           data['cats']['train'],
                           data['batches']['train'],
                           ...
                           triplet_dloss=args.triplet_dloss,
                           quantize=False, device=device)
    valid_set = MSDataset4(data['inputs']['valid'], 
                           data['meta']['valid'], 
                           data['names']['valid'],
                           data['cats']['valid'],
                           [x for x in data['batches']['valid']],
                           ...
                           random_recs=False, triplet_dloss=args.triplet_dloss,
                           quantize=False, device=device)
    test_set = MSDataset4(data['inputs']['test'], 
                          data['meta']['test'], 
                          data['names']['test'],
                          data['cats']['test'],
                          [x for x in data['batches']['test']],
    ...
    loaders = {
        'train': DataLoader(train_set,
                            num_workers=0,
                            # shuffle=True,
                            sampler=WeightedRandomSampler(samples_weights['train'], 
                                                          len(samples_weights['train']),
                                                            replacement=True),
                            batch_size=args.bs,
                            pin_memory=False,
                            drop_last=True),
...
def get_loaders_no_pool(data, random_recs, samples_weights, 
                        triplet_dloss, ae=None, 
                        classifier=None, bs=64,
                        device='cuda'):
    """
    ...
    train_set = MSDataset3(data['inputs']['train'], 
                           data['meta']['train'], 
                           data['names']['train'],
                           data['cats']['train'], 
                           ...
                           transform=transform, crop_size=-1, 
                           random_recs=random_recs, triplet_dloss=triplet_dloss)
    valid_set = MSDataset3(data['inputs']['valid'], 
                           data['meta']['valid'], 
                           data['names']['valid'],
                           data['cats']['valid'], 
                           [x for x in data['batches']['valid']], 
                           ...
                           random_recs=False, 
                           triplet_dloss=triplet_dloss)
    test_set = MSDataset3(data['inputs']['test'], 
                          data['meta']['test'], 
                          data['names']['test'], 
                          data['cats']['test'],
    ...
    loaders = {
        ...
        'test': DataLoader(test_set,
                           num_workers=0,
                           sampler=WeightedRandomSampler(samples_weights['test'], 
                                                         sum(samples_weights['test']),
                                                         replacement=False),
                           batch_size=bs,
                           pin_memory=False,
                           drop_last=False),
        'valid': DataLoader(valid_set,
                            num_workers=0,
                            sampler=WeightedRandomSampler(samples_weights['valid'], 
                                                          sum(samples_weights['valid']),
                                                          replacement=False),
                            batch_size=bs,
                            pin_memory=False,
                            drop_last=False),
...
class MSImages:
    ...
    def process(self, i):
        ...
        png = Image.open(f"{self.args.path}/{fname}")
        # if self.resize:
        # png = transforms.Resize((self.new_size, self.new_size))(png)
        png.load()  # required for png.split()

        new_png = Image.new("RGB", png.size, (255, 255, 255))
        new_png.paste(png, mask=png.split()[3])  # 3 is the alpha channel
        label = fname.split('_')[0]
        mat_data = np.asarray(new_png).copy()
        if self.binarize:
            mat_data[mat_data > 0] = 1

        return mat_data.transpose([2, 0, 1]), label, batch, plate, fname.split('.csv')[0]
...
class MSCSV:
    ...
    def process(self, i):
        ...
        try:
            ...
            mat_data = Image.open(f"{self.path}/{fname}/{fname}")
...
class MS2CSV:
    ...
    def process(self, i):
        ...
        print(f"Processing sample #{i}: {fname}")
        mat_datas = []
        for f in os.listdir(f"{self.args.path}/{fname}"):
            mat_data = read_csv(f"{self.args.path}/{fname}/{f}")
            if self.resize:
                try:
                    mat_data = transforms.Resize((self.new_size, self.new_size))(
                    torch.Tensor(mat_data.values).unsqueeze(0)).squeeze().detach().cpu().numpy()
