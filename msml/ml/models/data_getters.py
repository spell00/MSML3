...
from models.dataset import MSCSV, MS2CSV, MSImages
...
def split_images_ms2(data, args, seed=42):
    """
    Gets bacteria images or csv data. Data is automatically normalized using the maximum peak across all samples
    Args:
    ...
    data['inputs']['train'], data['inputs']['valid'], data['inputs']['test'] = data['inputs']['all'][train_inds], \
        data['inputs']['all'][valid_inds], data['inputs']['all'][test_inds]
    data['meta']['train'], data['meta']['valid'], data['meta']['test'] = data['meta']['all'][train_inds], \
        data['meta']['all'][valid_inds], data['meta']['all'][test_inds]
    data['labels']['train'], data['labels']['valid'], data['labels']['test'] = data['labels']['all'][train_inds], \
        data['labels']['all'][valid_inds], data['labels']['all'][test_inds]
    data['names']['train'], data['names']['valid'], data['names']['test'] = data['names']['all'][train_inds], \
        data['names']['all'][valid_inds], data['names']['all'][test_inds]
    data['orders']['train'], data['orders']['valid'], data['orders']['test'] = data['orders']['all'][train_inds], \
        data['orders']['all'][valid_inds], data['orders']['all'][test_inds]
    data['batches']['train'], data['batches']['valid'], data['batches']['test'] = data['batches']['all'][train_inds], \
        data['batches']['all'][valid_inds], data['batches']['all'][test_inds]
    data['cats']['train'], data['cats']['valid'], data['cats']['test'] = data['cats']['all'][train_inds], data['cats']['all'][
        valid_inds], data['cats']['all'][test_inds]
...
def get_bacteria_images_ms2(path, args, seed=42):
    ...
    for info in ['inputs', 'meta', 'names', 'labels', 'cats', 'batches', 'orders', 'sets']:
        data[info] = {}
        for group in ['all', 'train', 'test', 'valid']:
            data[info]['all'] = np.array([])
    if 'csv' in path:
        process = MS2CSV(args, scaler='none')
    else:
        process = MSImages(args, binarize=False)
    pool = multiprocessing.Pool(multiprocessing.cpu_count() - 1)
    data_images = pool.map(process.process, range(process.__len__()))
    images, labels, batches, plates, names = [x[0] for x in data_images], pd.Series(
        [x[1] for x in data_images]), pd.Series(
    ...
    data['orders']['all'] = plates.values
    data['meta']['all'] = data['inputs']['all']
    data['sets']['all'] = np.array([group for _ in data['names']['all']])

    pos = [i for i, name in enumerate(data['labels']['all'].flatten()) if 'pool' not in data['labels']['all'][i]]
    data['names']['all'] = data['names']['all'][pos]
    data['labels']['all'] = data['labels']['all'][pos]
    data['batches']['all'] = data['batches']['all'][pos]
    data['meta']['all'] = data['meta']['all'][pos]
    data['orders']['all'] = data['orders']['all'][pos]
    data['inputs']['all'] = data['inputs']['all'][pos]
    data['sets']['all'] = data['sets']['all'][pos]
    unique_labels = np.unique(data['labels']['all'])
    unique_batches = np.unique(data['batches']['all'])
    # must be split based on batches, but batches should be plates
    # for group in ['train', 'valid', 'test', 'all']:
    data['batches']['all'] = np.array(
        [np.argwhere(unique_batches == x)[0][0] for x in data['batches']['all']]
    )
    # for group in ['train', 'valid', 'test', 'all']:
