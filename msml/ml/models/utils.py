...
def plot_confusion_matrix(cm, class_names, acc, mcc):
    ...
    plt.title(f"Confusion matrix (Acc: {np.round(np.mean(acc), 2)}, Mcc: {np.round(np.mean(mcc), 2)})")
    plt.colorbar()
    # plt.grid(b=None)
...
class LogConfusionMatrix:
    ...
    def plot(self, logger, epoch, unique_labels, mlops):
        ...
        for group in ['train', 'valid', 'test']:
            ...
            mcc = matthews_corrcoef(classes, preds)

            try:
                figure = plot_confusion_matrix(cm, 
                                               class_names=unique_labels[:len(np.unique(self.classes['train']))],
                                               acc=acc, mcc=mcc)
            except:
...
def log_confusion_matrix(logger, epoch, lists, unique_labels, traces, mlops):
    ...
    for values in list(lists.keys())[1:]:
        ...
        figure = plot_confusion_matrix(cm, class_names=unique_labels[:len(np.unique(classes))],
                                       acc=traces[values]['acc'], mcc=traces[values]['mcc'])
        if mlops == "tensorboard":
