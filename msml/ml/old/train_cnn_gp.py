...
class Predictor(nn.Module):
    ...
    def __init__(self, n_classes: int, n_batches: int):
        ...
        self.dann_discriminator = nn.Linear(32, n_batches)
        # self.transforms = weights.transforms()

    def forward(self, input: torch.Tensor) -> torch.Tensor:
        # x = self.transforms(input)
        enc = self.layer1(input)
        # x = self.classifier(x.view(x.size(0), -1))
        return enc.squeeze(-1).squeeze(-1)

class Predictor2(nn.Module):

    def __init__(self, n_classes: int, n_batches: int, n_neurons=4704, dropout=0.5):
        super().__init__()
        self.layer1 = nn.Sequential(
            # nn.Conv2d(28, 32, kernel_size=3, padding=1),
            # nn.ReLU(),
            nn.MaxPool2d(kernel_size=(5, 5), stride=5),
            nn.BatchNorm2d(28)
        )
        ...
        self.dann_discriminator = nn.Linear(n_neurons, n_batches)
        # self.transforms = weights.transforms()
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        enc = self.layer1(input)
        # x = self.classifier(x.view(x.size(0), -1))
        enc = enc.squeeze(-1).squeeze(-1).view(enc.size(0), -1)
        return enc
...
class TrainAE:
    ...
    def __init__(self, args, path, fix_thres=-1, load_tb=False, log_metrics=False, keep_models=True, log_inputs=True,
        ...
        self.args.log_tb = log_tb
        self.args.log_mlflow = log_mlflow
        self.args = args
        self.path = path
        ...
        self.data, self.unique_labels, self.unique_batches = get_bacteria_images_ms2(self.path, args)
    ...
    def train(self, params):
        ...
        while h < self.args.n_repeats:
            ...
            if len(event_acc.Tags()['tensors']) > 2 and self.load_tb:
                ...
                ae = Predictor2(n_classes=self.n_cats, 
                                n_batches=self.n_batches,
                                n_neurons=self.n_neurons,
                                dropout=dropout).to(self.args.device)
    ...
    def log_rep(self, best_lists, best_vals, best_values, traces, model, metrics, run, loggers, ae, shap_ae, h,
        ...
        if self.args.log_neptune:
            ...
            #                                    'enc',
            #                                    'neptune')
            #     if self.args.log_mlflow and self.pools:
            #         metrics = log_pool_metrics(enc_data['inputs'], enc_data['batches'], enc_data['labels'], run, epoch, metrics, 'enc',
            #                                    'mlflow')
            #         metrics = log_pool_metrics(enc_data['inputs'], enc_data['batches'], enc_data['labels'], model, epoch, metrics,
            #                                    'enc',
...
if __name__ == "__main__":
    ...
    parser.add_argument('--triplet_dloss', type=int, default=0, help='')
    ...
    train = TrainAE(args, args.path, fix_thres=-1, load_tb=False, 
